...
class EncoderLayer(nn.Module):
    def __init__(self, d_model, n_heads, d_ff, dropout):
        ...
        self.self_attn = MultiHeadAttention() # type: ignore # 占位符，后续实现
        self.feed_forward = PositionWiseFeedForward() # type: ignore # 占位符，后续实现
        self.norm1 = nn.LayerNorm(d_model)
        self.norm2 = nn.LayerNorm(d_model)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x, mask):
        # 残差链接与层归一化将在后续小节中详细解释
        # 1. 多头自注意力机制
        attn_output = self.self_attn(x, x, x, mask)
        x = self.norm1(x + self.dropout(attn_output))  # 残差链接与层归一化
...
class DecoderLayer(nn.Module):
    def __init__(self, d_model, n_heads, d_ff, dropout):
        ...
        self.self_attn = MultiHeadAttention() # type: ignore # 占位符，后续实现
        self.cross_attn = MultiHeadAttention() # type: ignore # 占位符，后续实现
        self.feed_forward = PositionWiseFeedForward() # type: ignore # 占位符，后续实现    
        self.norm1 = nn.LayerNorm(d_model)
        self.norm2 = nn.LayerNorm(d_model)
